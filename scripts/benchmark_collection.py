...
def select_collections_menu() -> List[str]:
    ...
    for i, col_name in enumerate(collections, 1):
        ...
        if metadata['chunk_size']:
            overlap_str = f"{metadata['overlap']*100:.0f}%" if metadata['overlap'] else "N/A"
            print(f"       Paramètres : {metadata['chunk_size']} mots, {overlap_str} overlap")
...
def benchmark_collections(collections_to_test: List[str],
                         query: str,
                         n_results: int = 5,
        ...
        try:
            ...
            if verbose:
                ...
                if params['chunk_size']:
                    print(f"   • Paramètres : {params['chunk_size']} mots, {params['overlap']*100:.0f}% overlap")
...
def benchmark_single_query(query: str = "Quels sont les dangers de l'intelligence artificielle ?",
                           n_results: int = 5,
...
def benchmark_multiple_queries(queries: List[str] = None,
                               n_results: int = 5,
...
def print_comparison_table(results: List[Dict[str, Any]], title: str = "RÉSULTATS"):
    ...
    # En-tête
    print(f"{'Collection':<25} {'Size':<8} {'Overlap':<10} {'Chunks':<10} "
    ...
    for r in results:
        ...
        overlap_str = f"{r['overlap']*100:.0f}%" if r['overlap'] is not None else "N/A"
        
        print(f"{r['config_name']:<25} {size_str:<8} {overlap_str:<10} "
    ...
    print(f"    Meilleur score : {best_score_config['config_name']} ({best_score_config['best_score']:.3f})")
    print(f"   ⚡ Plus rapide : {fastest_config['config_name']} ({fastest_config['query_time']:.3f}s)")
    print(f"   Plus compacte : {smallest_config['config_name']} ({smallest_config['total_chunks']} chunks)")
    
    # Recommandation équilibrée
    balanced = max(results, key=lambda x: (x['best_score'] * 0.6) + (1 - x['query_time']/5) * 0.4)
    print(f"\n RECOMMANDATION (équilibre score + vitesse) : {balanced['config_name']}")
    if balanced['chunk_size']:
        print(f"   Paramètres : {balanced['chunk_size']} mots, {balanced['overlap']*100:.0f}% overlap")
...
def print_multi_query_summary(all_results: Dict[str, List[Dict[str, Any]]]):
    ...
    for query, results in all_results.items():
        ...
        print(f"   Meilleure : {best['config_name']} (score: {best['best_score']:.3f}, temps: {best['query_time']:.3f}s)")
    ...
    # Moyennes
    collection_averages = {
        col: sum(scores) / len(scores) 
        for col, scores in collection_scores.items()
    }
    
    # Tri décroissant
    sorted_collections = sorted(
        collection_averages.items(), 
        key=lambda x: x[1], 
        reverse=True
    )
...
def export_results(results: List[Dict[str, Any]], filename: str = "benchmark_results.txt"):
...
def export_multi_query_results(all_results: Dict[str, List[Dict[str, Any]]], 
