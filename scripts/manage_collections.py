...
def add_collection_interactive(client):
    ...
    if nom_collection in collections:
        print(f" Une collection '{nom_collection}' existe déjà ({client.get_collection(nom_collection).count()} docs)")
    ...
    chunk_size_input = input("   Taille des chunks EN CARACTÈRES (défaut: 1000) : ").strip()
...
def rename_collection(old_name: str, new_name: str, client) -> bool:
    ...
    if old_metadata.get("chunk_size"):
        ...
        overlap_str = f"{overlap} caractères" if overlap else "N/A"
        print(f"   Paramètres : {old_metadata.get('chunk_size')} mots, {overlap_str} overlap")
    ...
    for i in range(0, old_count, batch_size):
        try:
            ...
            print(f"    Batch {i//batch_size + 1}: {total_copied}/{old_count} docs ({progress:.1f}%)")
    ...
    if new_count != old_count:
        ...
        choix = input("Supprimer quand même l'ancienne collection ? (o/n) : ").strip().lower()
        if choix != 'o':
...
def add_documents_to_collection_interactive(r: Retrieval, client):
    ...
    chemin_source = input("\n Chemin vers le fichier ou le dossier à ajouter : ").strip()
    ...
    if path.is_file():
        ...
        files_to_check = [str(f) for f in path.rglob("*") if f.suffix.lower() in r.document_processor._handlers]

    duplicates = [
        f for f in files_to_check 
        if Path(f).resolve().relative_to(r.path_doc.resolve()).as_posix() in existing_files
    ]
    overwrite_flag = False # Par défaut, on ignore les doublons
    
    if duplicates:
        print(f"\n {len(duplicates)} doublon(s) détecté(s).")
        print("   1. Remplacer les doublons (supprimer les anciens + ajouter les nouveaux)")
        print("   2. Ignorer les doublons (ajouter uniquement les nouveaux fichiers) [Défaut]")
    ...
    if success:
        ...
        print(f"  Statistiques de '{collection_name}' : {final_stats['total_documents']} chunks, {final_stats['total_fichiers']} fichiers.")
    else:
        print("\n✗ Échec de l'opération d'ajout. Consultez les logs pour plus de détails.")
        
    return success

def search_sources_by_keyword(sources: list[dict], keyword: str, max_results: int = 10) -> list[dict]:
    """
   ...
   # 1. Lister les collections disponibles
    ...
    if choix.isdigit():
        ...
        if 1 <= num <= len(collections):
            collection_name = collections[num - 1]  # ← VOILÀ collection_name est défini !
        else:
...
def batch_create_collections(client):
    ...
    print("=" * 100)
    nb_collections_input = input(" Combien de collections voulez-vous créer ? : ").strip()
    ...
    for i in range(nb_collections):
        ...
        nb_md = len(list(source_path.rglob("*.md"))) + len(list(source_path.rglob("*.markdown")))
    ...
    for i, config in enumerate(collections_configs, 1):
        ...
        print(f"   • Chunking : {config['chunk_size']} caractères, {config['overlap']} caractères overlap")
        if config['ecraser']:
    ...
    for i, config in enumerate(collections_configs, 1):
        ...
        try:
            ...
            if success:
                ...
                print(f"   ⏱  Temps : {elapsed_time:.1f} secondes ({elapsed_time/60:.1f} minutes)")
    ...
    if collections_created:
        ...
        for col_info in collections_created:
            ...
            print(f"      • Temps : {col_info['temps']:.1f}s ({col_info['temps']/60:.1f} min)")
...
def manage_cache_menu(r: Retrieval):
    ...
    while True:
        ...
        if choix == '1':
            ...
            db_name = input("Nom de la base de données dont le cache est à vider (ex: DATA_Test) : ").strip()
            if db_name:
                r.document_processor.clear_cache(database=db_name)

        elif choix == '3':
            confirm = input("Voulez-vous vraiment vider TOUT le cache des textes extraits ? (tapez 'OUI') : ").strip()
            if confirm == "OUI":
...
def manage_collections():
    ...
    while True:
        ...
        if not collections:
            ...
            print("   Utilisez 'add_new_domain.py' pour créer votre première collection.")
            break
        ...
        for col_name in collections:
            ...
            if created_at != "N/A":
                date_only = str(created_at).split('T')[0] if 'T' in str(created_at) else created_at
        ...
        # Option 5 : Renommer une collection
            ...
            if new_name in collections:
                print(f" Le nom '{new_name}' est déjà utilisé par une autre collection.")
                input("\nAppuyez sur Entrée pour continuer...")
                continue
            
            # Confirmation
            confirmation = input(f"Confirmer le renommage de '{old_name}' en '{new_name}' ? (o/n) : ").strip().lower()
        ...
        # Option 2 : Supprimer plusieurs collections
            ...
            if non_existantes:
                ...
                continuer = input("Continuer avec les existantes ? (o/n) : ").strip().lower()
                if continuer != 'o':
                    continue
