...
class ChromaStorage:
    ...
    def switch_collection(self, collection_name: str):
        ...
        print(f" Collection active : {collection_name} ({self.collection.count()} docs)")
    ...
    def rename_collection(self, old_name: str, new_name: str) -> bool:
        ...
        try:
            ...
            new_col = self.chroma_client.create_collection(name=new_name, metadata=old_col.metadata)
            ...
            if count > 0:
                ...
                if len(data['ids']) > count:
                    for key in data:
                        if data[key]: data[key] = data[key][:count]
                ...
                if new_col.count() != count:
                    raise ValueError(f"La copie a échoué. Attendu: {count}, Obtenu: {new_col.count()}.")

            # 4. Si tout s'est bien passé, supprimer l'ancienne collection
            print(f"  Copie terminée. Suppression de l'ancienne collection '{old_name}'...")
            self.chroma_client.delete_collection(name=old_name)
            
            print(f"✓ [ChromaStorage] Renommage terminé avec succès.")
            return True
            ...
            try:
                ...
                print(f"  Nettoyage : La collection intermédiaire '{new_name}' a été supprimée.")
            except:
    ...
    def migrate_from_json(self, json_path: str) -> bool:
        ...
        for i in range(0, len(data), batch_size):
            ...
            try:
                ...
                print(f" Batch {i//batch_size + 1}: {total_migrated}/{len(data)} migrés")
    ...
    def query_similar(self, query_embedding: np.ndarray, n_results: int = 3) -> Tuple[List[str], List[str], List[float]]:
        try:
    ...
    def add_document(self, document: str, chemin: str, embedding: np.ndarray, position_debut: int = 0) -> bool:
        import uuid
    ...
    def get_stats(self) -> dict:
        ...
        """
        print(f"[ChromaStorage] Calcul des statistiques pour '{self.collection_name}'...")
    ...
    def migrate_paths_to_relative(self, root_data_path: Path) -> Tuple[int, int]:
        ...
        for i in range(len(data['ids'])):
            ...
            try:
                ...
                relative_path = current_absolute_path.relative_to(resolved_root).as_posix()
        ...
        if total_modified_count == 0:
            print("✓ Tous les chemins sont déjà correctement relatifs à la racine des données.")
            return 0, count
