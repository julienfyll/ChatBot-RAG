...
class PDFOCRProcessor:
    ...
    def has_text_layer(self, pdf_path: str) -> bool:
        ...
        print("Vérification de la présence de texte dans le PDF...")
        try:
            with open(pdf_path, "rb") as file:
                ...
                for page_num in range(pages_to_check):
                    ...
                    if text and len(text.strip()) > 50:
                        ...
                        return True
                print("✗ Aucun texte encodé trouvé (PDF scanné sans OCR)")
                return False
        except Exception as e:
            print(f" Erreur lors de la vérification : {e}")
            return False

    def detect_pdf_quality(
        self, pdf_path: str, min_words_in_line: int = 10, min_short_words: int = 5
    ) -> bool:
        """
        ...
        if not os.path.exists(pdf_path):
            raise FileNotFoundError(f"Le fichier {pdf_path} n'existe pas")
        print("Analyse de la qualité du PDF...")
        try:
            with open(pdf_path, "rb") as file:
                pdf_reader = PyPDF2.PdfReader(file)
                        ...
                        for word in words:
                            if len(word) <= 2 and word.isalpha():
                                ...
                                if consecutive_short >= min_short_words:
                                    print(
                                        f"✗ Mauvaise qualité détectée (page {page_num + 1})"
                                    )
                                    print(
                                        f"  Ligne problématique : {line_stripped[:80]}..."
                                    )
                                    return True
                            else:
                                consecutive_short = 0
                print("✓ Texte de bonne qualité")
                return False
        except Exception as e:
    ...
    def extract_text_and_post_process(self, pdf_path: str) -> str:
        ...
        print("Extraction du texte depuis le PDF (PyPDF2)...")
        full_text = ""
    ...
    def _clean_ocr_artifacts(self, text: str) -> str:
        ...
        text = re.sub(
            r"^[ÙùFfEeIiLlBb]\s+(?=Page|Mars|Juin|Par\.)", "", text, flags=re.MULTILINE
        )
            ...
            while i < len(words):
                ...
                if len(word) == 1 and word.isalpha():
                    ...
                    while (
                        j < len(words)
                        and len(words[j]) <= 2
                        and any(c.isalpha() for c in words[j])
                    ):
    ...
    def _remove_spurious_lines(self, text: str) -> str:
        ...
        for line in lines:
            ...
            if len(line_stripped) > 20:
                ...
                continue
            if re.match(r"^[A-Z][a-z]+\s+\d{2}$", line_stripped):
                continue
            if re.match(r"^Par\.\s*\d+$", line_stripped):
                continue
            if re.match(r"^(?:Page|p\.?)\s*\d+$", line_stripped, re.IGNORECASE):
                continue
            if re.match(r"^[\s\-_=+*#~\.,:;!?—–]{3,}$", line_stripped):
                continue
            if len(line_stripped) <= 2 and not line_stripped.isdigit():
                continue
            if re.match(r"^[A-Za-z\s]{1,5}$", line_stripped):
                continue
            cleaned_lines.append(line)
        return "\n".join(cleaned_lines)
    ...
    def ocr_pdf(
        self, pdf_path: str, preprocess: bool = False, post_process: bool = True
    ) -> str:
        ...
        images = convert_from_path(
            pdf_path, dpi=self.dpi, thread_count=os.cpu_count() or 4
        )
        full_text = ""
