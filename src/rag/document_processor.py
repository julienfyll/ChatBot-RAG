...
class DocumentProcessor:
    ...
    def _get_cached_text_path(self, source_key: str) -> Path:
        ...
        cached_path = (
            self.processed_texts_dir / database_folder / "files" / f"{filename}.txt"
        )
        cached_path.parent.mkdir(parents=True, exist_ok=True)
        return cached_path

    def _is_cache_valid(
        self, source_key: str, source_file_path: Path, cached_file: Path
    ) -> bool:
        ...
        current_hash = self._compute_file_hash(
            source_file_path
        )  # On calcule le hash sur le fichier physique (chemin absolu)
        cached_hash = self.cache_metadata[source_key].get(
            "hash"
        )  # On compare avec le hash stocké sous la clé relative

        return cached_hash == current_hash

    def _save_text_to_cache(
        self, source_key: str, source_file_path: Path, text: str, method: str
    ):
    ...
    def _load_text_from_cache(
        self, source_key: str, source_file_path: Path
    ) -> Optional[str]:
        database_folder = Path(source_key).parts[0]
        self._switch_cache_database(database_folder)  # Utilise _switch_cache_database
    ...
    def _get_database_folder(self, source_file: Path) -> Path:
        try:
            ...
            database_folder = (
                relative_path.parts[0] if relative_path.parts else "default"
            )
    ...
    def process_documents(
        ...
        for fichier in fichiers_a_traiter:
            ...
            handler = self._handlers.get(
                fichier.suffix.lower()
            )  # Utiliser .lower() pour la robustesse

            if handler:
                try:
                    texte_extrait, methode = handler(fichier)
                    if texte_extrait and texte_extrait.strip():
                        self._save_text_to_cache(
                            chemin_a_stocker, fichier, texte_extrait, methode
                        )
                        textes.append(texte_extrait)
                        chemins.append(chemin_a_stocker)
                    else:
                        print(
                            f"    /!\\ Fichier ignoré (aucun texte extrait) : {fichier.name}"
                        )
                except Exception as e:
                    print(
                        f"    /!\\ Erreur lors du traitement de {fichier.name} avec le handler '{fichier.suffix}' : {e}"
                    )
            else:
                print(f"    /!\\ Type de fichier non supporté : {fichier.suffix}")

        print(
            f"[DocumentProcessor] Statistiques du cache : {cache_hits} hits, {cache_misses} misses."
        )
    ...
    def _process_pdf(self, file_path: Path) -> Tuple[str, str]:
        ...
        if not self.ocr_processor.has_text_layer(
            str(file_path)
        ) or self.ocr_processor.detect_pdf_quality(str(file_path)):
    ...
    def list_cache(self):
        ...
        for db_folder in sorted(databases):
            ...
            for source_path, meta in metadata.items():
                ...
                if cached_file.exists():
                    ...
                    db_size += size_kb
                    print(
                        f"    - {Path(source_path).name} ({meta['method']}, {size_kb:.1f} KB)"
                    )
        ...
        print("\n" + "=" * 100)
        print(
            f"  TOTAL : {total_files} fichier(s) dans {len(databases)} base(s), {total_size / 1024:.1f} MB"
        )
        print("=" * 100)
    ...
    def rebuild_cache_for_file(self, source_file: str):
        ...
        self.process_documents(
            fichiers_specifiques=[str(source_path)], force_reprocess=True
        )
        ...
        for db_folder in databases:
            ...
            for old_key, value in old_metadata.items():
                try:
                    ...
                    new_metadata[old_key] = (
                        value  # On conserve l'ancienne clé en cas d'erreur
                    )
