...
def select_collection_menu(rag_instance):
    ...
    # Menu de sélection
    DEFAULT_COLLECTION = "CGT_MPNet4"  # ← Changez ce nom selon votre collection préférée
...
def test_complete():
  ...
  # Vérifier que ChromaDB a des collections
    ...
    if metadata.get("chunk_size"):
        print(f"   • Paramètres : {metadata.get('chunk_size')} mots, {metadata.get('overlap', 0)*100:.0f}% overlap")
