...
st.set_page_config(page_title="Assistant RAG CGT", page_icon="🤖", layout="wide")
...
def main():
    ...
    with st.sidebar:
        ...
        selected_collection = st.selectbox("Collection :", collections, index=0)
        ...
        if selected_collection:
            ...
            with st.expander(" Détails de la collection"):
                if metadata.get("chunk_size"):
                    st.write(
                        f"**Taille chunks :** {metadata.get('chunk_size')} caractères"
                    )
    ...
    with st.sidebar:
        ...
        if st.button(" Réinitialiser la session LLM"):
            try:
                ...
                st.success(
                    " Session LLM réinitialisée ! L'historique des conversations a été effacé."
                )
                st.info(
                    " La mémoire du modèle est maintenant vide, vous pouvez poser de nouvelles questions sans risque de dépassement de tokens."
                )
            except Exception as e:
